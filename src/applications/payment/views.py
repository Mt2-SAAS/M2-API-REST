...
Paymentwall.set_app_key(
    settings.PAYMENTWALL_PUBLIC_KEY
)  # available in your merchant area
Paymentwall.set_secret_key(
    settings.PAYMENTWALL_PRIVATE_KEY
)  # available in your merchant area
    ...
    def get(self, request, code):
        ...
        try:
            ...
            if not code.status:
                code.use_code(user)
                return Response(
                    {"status": "Codigo Aplicado"}, status=status.HTTP_200_OK
                )

            return Response(
                {"status": "Codigo Usado"}, status=status.HTTP_400_BAD_REQUEST
            )

        except PaymentCode.DoesNotExist:
            return Response(
                {"status": "Codigo no encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if pingback.validate():
            ...
            if pingback.is_deliverable():
                ...
                try:
                    ...
                    paylogs = PaymentLogs.objects.get(
                ...
                logger.error(
                    "Paymentwall pingback: Unknown pingback type, Paymentwall sent this data: {}".format(
                        request.GET.copy()
                    )
                )
            ...
            logger.error(
                "Paymentwall pingback: Cant validate pingback, error: {} Paymentwall sent this data: {} IP {} ".format(
                    pingback.get_error_summary(),
                    request.GET.copy(),
                    self.__get_request_ip(),
                )
            )
